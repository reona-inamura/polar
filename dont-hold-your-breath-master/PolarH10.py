...
class PolarH10:
    ...
    def __init__(self, bleak_device):
        ...
        self.ibi_data = None
        self.ecg_data = None
        self._pmd_enabled = False#stop_notifyを二回呼ばないためにあとから追加した。正直要らない気もする
        self.warmup_time = 5.0 # Polar H10のウォームアップ時間

        self.acc_stream_timestamp = []
        self.ibi_stream_timestamp = []
        self.ecg_stream_timestamp = []
    ...
    # 共通通知ハンドラ
    def _pmd_data_conv(self, sender, data):
        packet_type = data[0]
        if packet_type == 0x02:
            # ACC パケット
    ...
    # PMD ストリーム開始（ACC+ECG）
    async def start_pmd_stream(self):
        if self._pmd_enabled:
            return
        # ACC と ECG の両方の開始コマンドを送信
        # （あるいは別々に write しても OK）
        await self.bleak_client.write_gatt_char(
            PolarH10.PMD_CHAR1_UUID,
            PolarH10.ACC_WRITE,
            response=True
        )
        await self.bleak_client.write_gatt_char(
        ...
        await self.bleak_client.start_notify(
            PolarH10.PMD_CHAR2_UUID,
            self._pmd_data_conv
        )
        self._pmd_enabled = True
        print("Collecting ACC + ECG data...", flush=True)

    # PMD ストリーム停止
    async def stop_pmd_stream(self):
        if not self._pmd_enabled:
            return
        await self.bleak_client.stop_notify(PolarH10.PMD_CHAR2_UUID)
        self._pmd_enabled = False
        print("Stopping ACC + ECG data...", flush=True)

    def get_acc_data(self):
        
        acc_times = np.array(self.acc_stream_times)
        acc_timestamp = acc_times
        acc_times = acc_times - acc_times[0] # rel to start of acc session
        self.acc_data = {'times': acc_times, 'values': np.array(self.acc_stream_values), 'timestamp': acc_timestamp}
    ...
    def get_ibi_data(self):
        ...
        ibi_timestamp = ibi_times
        ibi_times = ibi_times - self.acc_stream_start_time # rel to start of acc session time in unix s
        self.ibi_data = {'times': ibi_times, 'values': np.array(self.ibi_stream_values), 'timestamp': ibi_timestamp}

        return self.ibi_data
    # 追加：ECGデータ取得
    def get_ecg_data(self):
        # タイムスタンプを最初をゼロにして相対時刻化
        ecg_times = np.array(self.ecg_stream_times)
        ecg_timestamp = ecg_times
        ecg_times = ecg_times - ecg_times[0]
        self.ecg_data = {
            'times': ecg_times,
